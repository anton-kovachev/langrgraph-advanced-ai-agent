...
Please analyze these Bing results and extract insights that complement other search sources."""
    ...
    @staticmethod
    def yandex_analysis_system() -> str:
        """System prompt for analyzing Yandex search results."""
        return """You are an expert research analyst. Analyze the provided Yandex search results to extract complementary insights that answer the user's question.
...
Provide a concise analysis highlighting unique findings and perspectives."""
    ...
    @staticmethod
    def yandex_analysis_user(user_question: str, yandex_results: str) -> str:
        """User prompt for analyzing Yandex search results."""
        return f"""Question: {user_question}

Yandex Search Results: {yandex_results}

Please analyze these Yandex results and extract insights that complement other search sources."""
...
DuckduckGo Analysis: {yandex_analysis}
...
def get_yandex_analysis_messages(
    user_question: str, yandex_results: str
) -> list[Dict[str, Any]]:
    """Get messages for Bing results analysis."""
    return create_message_pair(
        PromptTemplates.yandex_analysis_system(),
        PromptTemplates.yandex_analysis_user(user_question, yandex_results),
    )
...
def get_synthesis_messages(
    user_question: str, google_analysis: str, bing_analysis: str, yandex_analysis: str, reddit_analysis: str
) -> list[Dict[str, Any]]:
    """Get messages for final synthesis."""
    return create_message_pair(
        PromptTemplates.synthesis_system(),
        PromptTemplates.synthesis_user(
            user_question, google_analysis, bing_analysis, yandex_analysis, reddit_analysis
        ),
    )